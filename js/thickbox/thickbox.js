...
function TB_show(caption, url, imageGroup) {//function called when the user clicks on a thickbox link
	...
	try {
		...
		$("body").append("<div id='TB_load'><img src='" + thickboxURL + "/loadingAnimation.gif' /></div>");
		TB_load_position();
		...
		if(urlType=='.htm'||urlType=='.html'||urlType=='.php'||urlType=='.asp'||urlType=='.aspx'||urlType=='.jsp'||urlType=='.jst'||urlType=='.rb'||urlType=='.txt'||urlType=='.cfm' || (url.indexOf('TB_inline') != -1) || (url.indexOf('TB_iframe') != -1) ){//code to show html pages
			...
			if(url.indexOf('TB_iframe') != -1){				
					...
					$("#TB_window").append("<div id='TB_title'><div id='TB_ajaxWindowTitle'>"+caption+"</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton'>close</a></div></div><iframe src='"+url+"' id='TB_iframeContent' style='width:"+(ajaxContentW + 30)+"px;height:"+(ajaxContentH + 18)+"px;'></iframe>");
				}else{
...
function TB_position() {
	...
	var TB_top = arrayPageScroll[1] + ((pagesize[1]-TB_HEIGHT)/2);
	if( TB_top < 0 ) TB_top = 20;
	$("#TB_window").css({width:TB_WIDTH+"px",left: ((pagesize[0] - TB_WIDTH)/2)+"px", top: TB_top +"px" });
	TB_overlaySize();
}
