 ...
 */
$VM_LANG->PHPSHOP_EPAY_FRAUD = 'Der er blevet lavet: %s betaling(er) med dette kort de sidste 24 timer';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_DATE = 'Dato for betalingen: ';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_FEE = 'Gebyr pålagt betalingen: ';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_CALLBACK = '<b>Ordre betalingsinfo blev opdateret via callback!</b>';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_ORDER_COMMENT = 'Betalingen blev godkendt af ePay / PBS. \nBetalingen har fået følgende transaktionsnummer i ePay.: ';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_RETRY_PAYMENT = '<br/><br><br><b>Tryk på nederstående link for at forsøg betaling igen:</b><br>';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_ORDER_NOT_FOUND = '(Ordren blev ikke fundet - Intern fejl)';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_MD5_CHECK_FAILURE = '(Fejl i MD5 validering - Intern fejl - konfigurations fejl)';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_DECLINE = 'Betalingen blev ikke godkendt af ePay / PBS. Betalingen blev afvist pga. følgende fejlkode: ';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_LOG_TID = 'Betalingen er oprettet med transaktionsnummer: ';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_EPAY_LINK = '. Betalingen kan behandles på: <a href="https://ssl.ditonlinebetalingssystem.dk/admin" target="_new">www.epay.dk</a>';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_CHECKOUT_HEADER = 'Du har nu bekr&#230;ftet ordren og skal nu v&#230;lge hvordan ordren betales. Dette g&#248;res ved at klikke p&#229; den &#248;nskede betalingsmetode nedenfor. Betalingen vil ske i et SSL sikret vindue hos ePay (<a href="http://www.epay.dk" target="_new">www.epay.dk</a>).<br><br>';
$VM_LANG->PHPSHOP_EPAY_PAYMENT_CHECKOUT_FOOTER = 'Hvis ikke Standard Betalingsvinduet &#229;bner op automatisk, så klik på knappen for at aktivere det. <br><br> Bem&#230;rk! Hvis I benytter en pop-up stopper, skal I holde CTRL tasten nede, mens I trykker p&#229; knappen.';        
$VM_LANG->PHPSHOP_EPAY_BUTTON_OPEN_WINDOW = '&Aring;BEN BETALINGSVINDUET';
...
<table style="text-align: left;">
	<tr>
		...
		<td>The Merchant ID you have reveived from PBS. For testing purpose a
		test Merchant ID can be obtained by contacting ePay (sales@epay.dk)</td>
	</tr>
	<tr>
		...
		<td>Select the order status to which the actual order is set, if the
		Transaction was successful. If using download selling options: select
		the status which enables the download (then the customer is instantly
		notified about the download via e-mail).</td>
	</tr>
	<tr>
	...
	<tr>
		...
		EPAY_GROUP ;
		?>"></td>
		<td>To move the payment to a specified group. If not leave this field
		empty.</td>
	</tr>
	<tr>
		...
		if( EPAY_MD5_TYPE == '0' )
			echo "selected=\"selected\"" ;
		?>
				value="0">Disabled (0)</option>
			<option
				<?php
		if( EPAY_MD5_TYPE == '1' )
			echo "selected=\"selected\"" ;
		?>
				value="1">MD5 on accept page (1)</option>
			<option
				<?php
		if( EPAY_MD5_TYPE == '2' )
			echo "selected=\"selected\"" ;
		?>
				value="2">MD5 on both data transmitted to ePay and on accept page
			(2)</option>
		</select></td>
		<td>The type of MD5 usage. 0 disables MD5. 1 uses MD5 on the accept
		page on this shop. 2 creates a MD5 stamp, which is send to ePay with
		transaction data. If this stamp does not match ePay will decline the
		payment.</td>
	</tr>
	<tr>
		...
		EPAY_MD5_KEY ;
		?>"></td>
		<td>If MD5 is enabled above the secure key for MD5 stamping is defined
		here (same secret key must be defined in the ePay administration). If
		MD5 is disabled leave this field empty.</td>
	</tr>
	<tr>
		...
		EPAY_AUTH_SMS ;
		?>"></td>
		<td>Enter a mobile number here to receive a SMS message when the
		payment is verified by ePay. <b>Notice! each SMS costs DKK 0.25.</b></td>
	</tr>
	<tr>
		...
		EPAY_AUTH_MAIL ;
		?>"></td>
		<td>Enter one or more email adresses to receive an email when the
		payment is verified by ePay. For multiple receivers seperate the
		emailaddresses by semicolon (;).
	...
	<tr>
		...
		if( EPAY_WINDOW_STATE == '1' )
			echo "selected=\"selected\"" ;
		?>
				value="1">Popup (1)</option>
			<option
				<?php
		if( EPAY_WINDOW_STATE == '2' )
			echo "selected=\"selected\"" ;
		?>
				value="2">Same window (full screen) (2)</option>
		</select></td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		...
		if( EPAY_3DSECURE == '1' )
			echo "selected=\"selected\"" ;
		?>
				value="1">3D secure is enabled (1)</option>
			<option
				<?php
		if( EPAY_3DSECURE == '2' )
			echo "selected=\"selected\"" ;
		?>
				value="2">Only payments by use of 3D secure is allowed (2)</option>
			<option
				<?php
		if( EPAY_3DSECURE == '3' )
			echo "selected=\"selected\"" ;
		?>
				value="3">3D secure payments are disabled (3)</option>
		</select></td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		...
		<td>Callback is used to avoid missing update of the shopping system if
		the user closes the browser before redirect to the shop from ePay is
		made. <b>(It is recommented that callback is enabled to update the
		shop even if the customer during the payment process closes the
		browser)</b></td>
	</tr>
	<tr>
		...
		if( EPAY_ADDFEE == '0' )
			echo "selected=\"selected\"" ;
		?>
				value="0">Customer does not pay for the transaction fee (0)</option>
			<option
				<?php
		if( EPAY_ADDFEE == '1' )
			echo "selected=\"selected\"" ;
		?>
				value="1">The customer paies for the transaction fee (1)</option>
		</select></td>
		<td>Often it is necessary to put the transaction fee to the customer.
		ePay will calculate the fee and put it on the payment before the
		payment is made.</td>
	</tr>
	<tr>
		...
		if( EPAY_SUBSCRIPTION == '0' )
			echo "selected=\"selected\"" ;
		?>
				value="0">Disable subscription (0)</option>
			<option
				<?php
		if( EPAY_SUBSCRIPTION == '1' )
			echo "selected=\"selected\"" ;
		?>
				value="1">All payments are created as recurrent subscriptions (1)</option>
		</select></td>
		<td>Subscriptions are used for creating payments that can be
		authorized more than once (recurrent payments)</td>
	</tr>
