...
class plgShippingDhl extends vmShippingPlugin {
	...
	function get_shipping_rate_list(&$d) {
		...
		if ($cur_day_of_week == 'Sun') {
			...
			$ship_delay_msg =
			    JText::_('JM_SHIPPING_METHOD_DHL_NOT_ON_WEEKENDS') . " " .
			    JText::_('JM_SHIPPING_METHOD_DHL_WILL_GO_OUT') . ": " .
			...
			if ($cur_time > intval(DHL_TOO_LATE)) {
				...
				if ($cur_day_of_week == 'Fri') {
					...
					$ship_delay_msg =
						JText::_('JM_SHIPPING_METHOD_DHL_TOO_LATE_TO_SHIP')
					    . " " .
						JText::_('JM_SHIPPING_METHOD_DHL_WILL_GO_OUT')
					    . ": " .  date('M j, Y', $ship_timestamp);
					$ship_day = 'Mon';
		...
		if (!$is_international) {
			if ($this->params->get('DHL_EXPRESS_ENABLED') == 'TRUE') {
				$methods[] = array(
					'service_desc' =>
					    JText::_('JM_SHIPPING_METHOD_DHL_EXPRESS'),
					'service_code' => 'E',
					'special_service' => '',
			...
			if ($this->params->get('DHL_NEXT_AFTERNOON_ENABLED') == 'TRUE') {
				$methods[] = array(
					'service_desc' =>
					    JText::_('JM_SHIPPING_METHOD_DHL_NEXT_AFTERNOON'),
					'service_code' => 'N',
					'special_service' => '',
					'package_type' => $this->params->get('DHL_DOMESTIC_PACKAGE'),
					'international' => false);
			...
			if ($this->params->get('DHL_SECOND_DAY_ENABLED') == 'TRUE') {
				$methods[] = array(
					'service_desc' =>
					    JText::_('JM_SHIPPING_METHOD_DHL_SECOND_DAY'),
					'service_code' => 'S',
					'special_service' => '',
					'package_type' => $this->params->get('DHL_DOMESTIC_PACKAGE'),
					'international' => false);
			...
			if ($this->params->get('DHL_GROUND_ENABLED') == 'TRUE') {
				$methods[] = array(
					'service_desc' =>
					    JText::_('JM_SHIPPING_METHOD_DHL_GROUND'),
					'service_code' => 'G',
					'special_service' => '',
					'package_type' => $this->params->get('DHL_DOMESTIC_PACKAGE'),
					'international' => false);
			...
			if ($this->params->get('DHL_1030_ENABLED') == 'TRUE') {
				$methods[] = array(
					'service_desc' =>
					    JText::_('JM_SHIPPING_METHOD_DHL_1030'),
					'service_code' => 'E',
			...
			if (DHL_SATURDAY_ENABLED == 'TRUE' && ($ship_day == 'Fri')) {
				$methods[] = array(
					'service_desc' =>
					    JText::_('JM_SHIPPING_METHOD_DHL_SATURDAY'),
					'service_code' => 'E',
					'special_service' => 'SAT',
					'package_type' => $this->params->get('DHL_DOMESTIC_PACKAGE'),
					'international' => false);
			...
			if ($this->params->get('DHL_INTERNATIONAL_ENABLED') == 'TRUE') {
				$methods[] = array(
					'service_desc' =>
					    JText::_('JM_SHIPPING_METHOD_DHL_INTERNATIONAL'),
					'service_code' => 'IE',
					'special_service' => '',
...
//			$vmLogger->err($xmlReq->toNormalizedString());
			...
			if (!$xmlResp->parseXML($xmlResult, false, true)) {
				$vmLogger->err(
				    JText::_('JM_SHIPPING_METHOD_DHL_INVALID_XML') .
				    $xmlResult);
...
//			$vmLogger->err($xmlResp->toNormalizedString());
			...
			if ($result_code == NULL) {
				$vmLogger->debug(
				    JText::_('JM_SHIPPING_METHOD_DHL_MISSING_RESULT') .
...
//		$vmLogger->err($xmlReq->toNormalizedString());
		...
		$xmlResp =& new DOMIT_Lite_Document();
		if (!$xmlResp->parseXML($xmlResult, false, true)) {
			echo JText::_('JM_SHIPPING_METHOD_DHL_INVALID_XML') .
			    $xmlResult;
			return (false);
		}
//		$vmLogger->err($xmlResp->toNormalizedString());
...
//		$vmLogger->err($xmlResp->toNormalizedString());
		 ...
		 */
		$msg = '<p><strong>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_HISTORY');
		$msg .= '</strong><br />';
		/* Now we've got all our data - format it into HTML */
		if (count($tracking) == 0) {
			$msg .= '<strong>';
			$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_NO_DATA');
			$msg .= '</strong>';
		} else {
		...
		if ($have_signature) {
			...
			$msg .= '<p>';
			$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_SIGNATURE_LEGEND');
			$msg .= '<ul>';
			$msg .= "\n";
			$msg .= '<li>';
			$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_LEGEND_LD');
			$msg .= '</li>';
			$msg .= "\n";
			$msg .= '<li>';
			$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_LEGEND_FD');
			$msg .= '</li>';
			$msg .= "\n";
			$msg .= '<li>';
			$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_LEGEND_SD');
			$msg .= '</li>';
			$msg .= "\n";
			$msg .= '<li>';
			$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_LEGEND_BD');
			$msg .= '</li>';
			$msg .= "\n";
			$msg .= '<li>';
			$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_LEGEND_GAR');
			$msg .= '</li>';
			$msg .= "\n";
			$msg .= '<li>';
			$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_LEGEND_LOF');
			$msg .= '</li>';
			$msg .= "\n";
			$msg .= '<li>';
			$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_LEGEND_LPN');
			$msg .= '</li>';
			$msg .= "\n";
			$msg .= '</ul>';
		...
		/* shipment */
		$msg .= '<strong>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_NUMBER') . ': ';
		$msg .= '</strong>';
		$msg .= ' ';
		$msg .= $tracking_number;
		$msg .= '<br />';
		...
		$msg .= '<strong>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_PACKAGE') . ': ';
		$msg .= '</strong>';
		$msg .= ' ';
		...
		$msg .= '<strong>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_SERVICE') . ': ';
		$msg .= '</strong>';
		$msg .= ' ';
		...
		$msg .= '<tr>';
		$msg .= '<td><strong>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_SENDER');
		$msg .= '</strong></td>';
		$msg .= '<td><strong>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_RECEIVER');
		$msg .= '</strong></td>';
		$msg .= '</tr>';
		...
		$msg .= '<td>';
		$msg .= '<strong>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_PICKUP');
		$msg .= '</strong>';
		$msg .= '</td>';

		$msg .= '<td>';
		$msg .= '<strong>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_DELIVERY');
		$msg .= '</strong>';
		$msg .= '</td>';
		$msg .= '</tr>';
		$msg .= "\n";
		...
		$msg .= '<tr>';
		$msg .= '<td>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_DATE') . ': ';
		...
		$msg .= '<tr>';
		$msg .= '<td>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_TIME') . ': ';
		...
		$msg .= '<tr>';
		$msg .= '<td>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_LOCATION') . ': ';
		...
		$msg .= '<tr>';
		$msg .= '<td>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_EST_DEL') . ': ';
		if (array_key_exists('est_delivery', $pickup))
			$msg .= $pickup['est_delivery'];
		$msg .= '</td>';
		$msg .= '<td>';
		$msg .= JText::_('JM_SHIPPING_METHOD_DHL_TRACKING_SIGNATORY') . ': ';
