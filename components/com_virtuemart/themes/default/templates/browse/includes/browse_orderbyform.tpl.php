...
<?php
if( !empty( $JM_BROWSE_ORDERBY_FIELDS )) {
...
echo mm_writeWithJS('&nbsp;<input type="hidden" name="DescOrderBy" value="'.$asc_desc[0].'" /><a href="javascript: document.order.DescOrderBy.value=\''.$asc_desc[1].'\'; document.order.submit()"><img src="'. $mosConfig_live_site."/images/M_images/$icon"  .'" border="0" alt="'. JText::_('JM_PARAMETER_SEARCH_'.$asc_desc[0].'ENDING_ORDER') .'" title="'.JText::_('JM_PARAMETER_SEARCH_'.$asc_desc[0].'ENDING_ORDER') .'" width="12" height="12" /></a>',
      '<select class="inputbox" name="DescOrderBy">
            <option '.$selected[0].' value="DESC">'.JText::_('JM_PARAMETER_SEARCH_DESCENDING_ORDER').'</option>
            <option '.$selected[1].' value="ASC">'.JText::_('JM_PARAMETER_SEARCH_ASCENDING_ORDER').'</option>
        </select>
        <input class="button" type="submit" value="'.JText::_('JM_SUBMIT').'" />');
}
?>
...
if( $show_top_navigation ) {
	...
	<noscript><input type="submit" value="<?php echo JText::_('JM_SUBMIT') ?>" /></noscript>
