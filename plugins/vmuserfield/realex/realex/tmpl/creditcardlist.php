	...
	foreach ($storedCreditCards as $storedCreditCard) {
		...
		<div class="releax_show_cc">
			...
			<div class="vmpayment_cc_info vmpayment_cc_date">
				...
				<?php
				$exp_date = explode("/", $storedCreditCard->realex_saved_pmt_expdate);
				if (count($exp_date) == 2) {
					echo shopfunctions::listMonths('cc_expire_month_' . $storedCreditCard->id, $exp_date[0], "class=\"inputbox vm-chzn-select\" style=\"width: 100px;\"", 'm');
					echo shopfunctions::listYears('cc_expire_year_' . $storedCreditCard->id, $exp_date[1], null, null, "class=\"inputbox vm-chzn-select\" style=\"width: 100px;\"  onchange=\"var month = document.getElementById('cc_expire_month_'" . $viewData['virtuemart_paymentmethod_id'] . "); if(!CreditCardisExpiryDate(month.value,this.value, '" . $viewData['virtuemart_paymentmethod_id'] . "')){this.value='';month.value='';}\" ", "y");
				}
				?>
