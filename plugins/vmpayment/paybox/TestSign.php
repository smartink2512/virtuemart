...
if( isset( $_POST['signer']) ) {                            // si on a demande la signature
	$keyFile='/Applications/MAMP/htdocs/VM2/VM2024/z_private/paybox/epayment/pubkey-3.pem';
	$keyFile='/Applications/MAMP/htdocs/VM2/VM2024/logs/paybox/pubkey-3.pem';
	$keyFile='/homez.130/joomlavi/www/PAYBOX/logs/paybox/pubkey-3.pem';


	//$key = LoadKey( 'TestK004.prv.pem', false );            // chargement de la cle prive (de test, sans mot de passe)
    $key = LoadKey( $keyFile, false );            // chargement de la cle prive (de test, sans mot de passe)
    if( $key ) {
...
if( isset( $_POST['verifier']) ) {                          // si on a demande la verification
    ...
    else                       $status = "Erreur lors de la v�rification de la signature";
}
...
?>
    ...
    <tr>
      <td>My status = <?php echo $status ?></td>
    </tr>
    <tr>
      <td><input type="text" name="data" size="300" value="<?php echo $_POST['data'] ?>"></td>
    ...
    <tr>
      <td><input type="text" name="signeddata" size="300" value="<?php echo $_POST['signeddata'] ?>"></td>
