...
class KlikandpayHelperKlikandpaySubscribe extends KlikandpayHelperKlikandpay {
	...
	function getSubscribeProducts (VirtueMartCart $cart, $pbxTotalVendorCurrency) {
		...
		$klikandpay_parent_field = $this->_method->subscribe_customfield;
		vmdebug('KLIKANDPAY getSubscriptionData klikandpay_parent_field', $klikandpay_parent_field);
		...
		foreach ($products as $key => $product) {
			...
			vmdebug('KLIKANDPAY getSubscriptionData getProdCustomFields', $product_custom_fields);
			 $pbx_2mont = 0;
			$klikandpay_parent_field_found = false;
			foreach ($product_custom_fields as $product_custom_field) {
				if ($product_custom_field->custom_parent_id == $klikandpay_parent_field) {
					...
					$klikandpay_parent_field_found = true;
				}
			}

			if ($klikandpay_parent_field_found) {
				vmdebug('KLIKANDPAY getSubscribeProducts $klikandpay_parent_field_found', $pbx_freqs, $pbx_nbpaies);
				$pbx_2monts = $pbx_2mont * $product->quantity;
				$usbscribe_pbx_2mont += $this->getPbxAmount($pbx_2monts);

			}
			vmdebug('KLIKANDPAY getSubscriptionData BY PRODUCT', $subscribe);
		}
		if (!empty($pbx_freqs) and !empty($pbx_freqs)) {
			...
			if (count($pbx_freq) > 1 or count($pbx_nbpaie) > 1) {
				...
				if (count($pbx_freq)) {
					vmInfo('VMPAYMENT_KLIKANDPAY_ERROR_PBX_FREQ');
				}
				// Nombre de prélèvements (0 = toujours).
				if (count($pbx_nbpaie)) {
					vmInfo('VMPAYMENT_KLIKANDPAY_ERROR_PBX_NBPAIE');
				}
				vmInfo('VMPAYMENT_KLIKANDPAY_ERROR_SUBCRIBE');
			...
			if ($usbscribe_pbx_2mont) {
				...
				vmdebug('KLIKANDPAY getSubscriptionData TOTAL', $subscribe);
