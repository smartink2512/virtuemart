...
if (!class_exists('PaypalIAOHelperPaypalAIO')) {
	require(JPATH_SITE . '/plugins/vmpayment/paypal_aio/paypal_aio/helpers/paypalaio.php');
}
if (!class_exists('PaypalHelperCustomerData')) {
	require(JPATH_SITE . '/plugins/vmpayment/paypal_aio/paypal_aio/helpers/customerdata.php');
}
if (!class_exists('PaypalHelperPayPalStd')) {
	require(JPATH_SITE . '/plugins/vmpayment/paypal_aio/paypal_aio/helpers/paypalstd.php');
}
if (!class_exists('PaypalHelperPayPalExp')) {
	require(JPATH_SITE . '/plugins/vmpayment/paypal_aio/paypal_aio/helpers/paypalexp.php');
}
if (!class_exists('PaypalHelperPayPalHosted')) {
	require(JPATH_SITE . '/plugins/vmpayment/paypal_aio/paypal_aio/helpers/paypalhosted.php');
}
if (!class_exists('PaypalHelperPayPalApi')) {
	require(JPATH_SITE . '/plugins/vmpayment/paypal_aio/paypal_aio/helpers/paypalapi.php');
}
class plgVmPaymentPaypal_aio extends vmPSPlugin {
	...
	public function getVmPluginCreateTableSQL() {
		return $this->createTableSQL('PayPal AIO Table');
	}
		...
		foreach ($this->methods as $this->_currentMethod) {
			if ($this->_currentMethod->paypalproduct == 'exp') {
				...
				$productDisplayHtml = $this->renderByLayout('expproduct',
					array(
						'text' => JText::_('VMPAYMENT_PAYPAL_AIO_EXPCHECKOUT_AVAILABALE'),
						'img' => $product['img'],
						'link' => $product['link'],
		...
		foreach ($this->methods as $this->_currentMethod) {
			if ($this->_currentMethod->paypalproduct == 'exp') {
				...
				$html .= $this->renderByLayout('expcheckout',
					array(
						'text' => JText::_('VMPAYMENT_PAYPAL_AIO_EXPCHECKOUT_BUTTON'),
						'img' => $button['img'],
						'link' => $button['link'],
						'sandbox' => $this->_currentMethod->sandbox,
						'virtuemart_paymentmethod_id' => $this->_currentMethod->virtuemart_paymentmethod_id
	...
	function plgVmConfirmedOrder($cart, $order) {
		...
		$paypalInterface = $this->_loadPayPalInterface();
		$paypalInterface->writelog('order number: ' . $order['details']['BT']->order_number, 'plgVmConfirmedOrder', 'message');
		...
		if ($this->_currentMethod->paypalproduct == 'std') {
			...
			$payment = $this->_storePaypalInternalData($this->_currentMethod, $response, $order['details']['BT']->virtuemart_order_id, $cart->virtuemart_paymentmethod_id);
			...
			if ($success) {
				...
				if ($this->_currentMethod->payment_type == '_xclick-subscriptions' || $this->_currentMethod->payment_type == '_xclick-payment-plan') {
					...
					$response = $paypalInterface->getResponse();
					$this->_storePaypalInternalData($this->_currentMethod, $response, $order['details']['BT']->virtuemart_order_id, $cart->virtuemart_paymentmethod_id);
				}
				...
				$cart->setCartIntoSession();
				$this->customerData->clear();
				VmInfo('VMPAYMENT_PAYPAL_AIO_PAYMENT_NOT_VALID');
...
//			$this->customerData->clear();
	...
	function plgVmOnPaymentResponseReceived(&$html) {
		...
		$expresscheckout = JRequest::getInt('expresscheckout', '');
		if ($expresscheckout) {
			return;
	...
	function plgVmOnUserPaymentCancel() {
		...
		VmInfo(Jtext::_('VMPAYMENT_PAYPAL_AIO_PAYMENT_CANCELLED'));
	...
	function plgVmOnPaymentNotification() {
		...
		$paypalInterface = $this->_loadPayPalInterface();
		$paypalInterface->setOrder($order);
		$paypalInterface->writelog($paypal_data, 'PaymentNotification, paypal_data:', 'debug');
		$paypalInterface->writelog($order_number, 'PaymentNotification, order_number:', 'debug');
		$paypalInterface->writelog($payments[0]->virtuemart_paymentmethod_id, 'PaymentNotification, virtuemart_paymentmethod_id:', 'debug');
		$order_history = $paypalInterface->processIPN($paypal_data, $payments);
		if (!$order_history) {
			return false;
		} else {
			$this->_storePaypalInternalData($this->_currentMethod, $paypal_data, $virtuemart_order_id, $payments[0]->virtuemart_paymentmethod_id);
			$paypalInterface->writelog('new_status:' . $order_history['order_status'], 'plgVmOnPaymentNotification', 'message');
	...
	private function _storePaypalInternalData( $paypal_data, $virtuemart_order_id, $virtuemart_paymentmethod_id) {
		...
		*/
		...
		if ($paypal_data) {
			$response_fields['paypalresponse_raw'] = json_encode($paypal_data);
		}
	...
	protected function renderPluginName($activeMethod) {
		...
		if ($activeMethod->sandbox == 'sandbox') {
	...
	function plgVmOnShowOrderBEPayment($virtuemart_order_id, $payment_method_id) {
		...
		foreach ($payments as $payment) {
			$html .= '<tr class="row1"><td>' . JText::_('VMPAYMENT_PAYPAL_AIO_DATE') . '</td><td align="left">' . $payment->created_on . '</td></tr>';
    ...
    <a href="#" class="PayPalLogOpener" rel="' . $payment->id . '" >
        ...
        <span class="icon-nofloat vmicon vmicon-16-xml"></span>&nbsp;';
				$html .= JText::_('VMPAYMENT_PAYPAL_AIO_VIEW_TRANSACTION_LOG');
				$html .= '  </a>';
				$html .= ' </td></tr>';
			}
	...
	function plgVmOnStoreInstallPaymentPluginTable($jplugin_id) {
		...
		if ($this->_currentMethod->published) {
			...
			$sandbox = "";
			if ($this->_currentMethod->sandbox == 'sandbox') {
				$sandbox = 'SANDBOX_';
			...
			if ($this->_currentMethod->paypalproduct == 'std') {
				...
				if (empty ($this->_currentMethod->$param)) {
					$text = JText::sprintf('VMPAYMENT_PAYPAL_AIO_PARAMETER_REQUIRED', JText::_('VMPAYMENT_PAYPAL_AIO_' . $sandbox . 'MERCHANT'), $this->_currentMethod->payment_name, $this->_currentMethod->virtuemart_paymentmethod_id);
					vmError($text);
				}
			}
			if ($this->_currentMethod->paypalproduct == 'exp' OR $this->_currentMethod->paypalproduct == 'hosted' OR $this->_currentMethod->paypalproduct == 'api') {
				...
				if (empty ($this->_currentMethod->$param)) {
					$text = JText::sprintf('VMPAYMENT_PAYPAL_AIO_PARAMETER_REQUIRED', JText::_('VMPAYMENT_PAYPAL_AIO_' . $sandbox . 'USERNAME'), $this->_currentMethod->payment_name, $this->_currentMethod->virtuemart_paymentmethod_id);
					vmError($text);
				}
				$param = $sandbox_param . 'api_password';
				if (empty ($this->_currentMethod->$param)) {
					$text = JText::sprintf('VMPAYMENT_PAYPAL_AIO_PARAMETER_REQUIRED', JText::_('VMPAYMENT_PAYPAL_AIO_' . $sandbox . 'PASSWORD'), $this->_currentMethod->payment_name, $this->_currentMethod->virtuemart_paymentmethod_id);
					vmError($text);
				}
				...
				if ($this->_currentMethod->authentication == 'signature') {
					...
					if (empty ($this->_currentMethod->$param)) {
						$text = JText::sprintf('VMPAYMENT_PAYPAL_AIO_PARAMETER_REQUIRED', JText::_('VMPAYMENT_PAYPAL_AIO_' . $sandbox . 'SIGNATURE'), $this->_currentMethod->payment_name, $this->_currentMethod->virtuemart_paymentmethod_id);
						vmError($text);
					}
				} else {
					$param = $sandbox_param . 'api_certificate';
					if (empty ($this->_currentMethod->$param)) {
						$text = JText::sprintf('VMPAYMENT_PAYPAL_AIO_PARAMETER_REQUIRED', JText::_('VMPAYMENT_PAYPAL_AIO_' . $sandbox . 'CERTIFICATE'), $this->_currentMethod->payment_name, $this->_currentMethod->virtuemart_paymentmethod_id);
						vmError($text);
					}
				}
			}
			if ($this->_currentMethod->paypalproduct == 'hosted') {
				...
				if (empty ($this->_currentMethod->$param)) {
					$text = JText::sprintf('VMPAYMENT_PAYPAL_AIO_PARAMETER_REQUIRED', JText::_('VMPAYMENT_PAYPAL_AIO_' . $sandbox . 'PAYFLOW_PARTNER'), $this->_currentMethod->payment_name, $this->_currentMethod->virtuemart_paymentmethod_id);
					vmError($text);
				}
			}
			if ($this->_currentMethod->paypalproduct == 'exp' AND empty ($this->_currentMethod->expected_maxamount)) {
				$text = JText::sprintf('VMPAYMENT_PAYPAL_AIO_PARAMETER_REQUIRED', JText::_('VMPAYMENT_PAYPAL_AIO_EXPECTEDMAXAMOUNT'), $this->_currentMethod->payment_name, $this->_currentMethod->virtuemart_paymentmethod_id);
				vmError($text);
			}
	...
	public function plgVmOnUpdateOrderPayment(&$order, $old_order_status) {
		...
		if ($order->order_status != $this->_currentMethod->status_capture AND $order->order_status != $this->_currentMethod->status_refund) {
			return null;
		}
		//Load the payments
		...
		if ($this->_currentMethod->payment_action == 'Authorization' and $order->order_status == $this->_currentMethod->status_capture) {
			...
			if ($paypalInterface->DoCapture($payment)) {
				$paypalInterface->writelog(JText::_('VMPAYMENT_PAYPAL_AIO_API_TRANSACTION_CAPTURED'), 'plgVmOnUpdateOrderShipment', 'message', true);
				$this->_storePaypalInternalData($this->_currentMethod, $paypalInterface->getResponse(false), $order->virtuemart_order_id, $payment->virtuemart_paymentmethod_id);
			}

		} elseif ($order->order_status == $this->_currentMethod->status_refund OR $order->order_status == $this->_currentMethod->status_canceled) {
			$paypalInterface = $this->_loadPayPalInterface();
			$paypalInterface->setOrder($order);
			$paypalInterface->setTotal($order->order_total);
			$paypalInterface->loadCustomerData();
			if ($paypalInterface->RefundTransaction($payment)) {
				if ($this->_currentMethod->payment_type == '_xclick-subscriptions') {
					$paypalInterface->writelog(JText::_('VMPAYMENT_PAYPAL_AIO_SUBSCRIPTION_CANCELLED'), 'plgVmOnUpdateOrderPayment Refund', 'message', true);
				} else {
					//Mark the order as refunded
					// $order->order_status = $method->status_refund;
					$paypalInterface->writelog(JText::_('VMPAYMENT_PAYPAL_AIO_API_TRANSACTION_REFUNDED'), 'plgVmOnUpdateOrderPayment Refund', 'message', true);
				}
				$this->_storePaypalInternalData($this->_currentMethod, $paypalInterface->getResponse(false), $order->virtuemart_order_id, $payment->virtuemart_paymentmethod_id);
			}
		}
			...
			if ($this->checkConditions($cart, $this->_currentMethod, $cart->pricesUnformatted)) {
				...
				if ($this->_currentMethod->paypalproduct == 'api') {
					if (empty($this->_currentMethod->creditcards)) {
						$this->_currentMethod->creditcards = PaypalIAOHelperPaypalAIO::getPaypalCreditCards();
	...
	function plgVmOnCheckoutCheckDataPayment(VirtueMartCart $cart) {
		...
		//	vmInfo (JText::_ ('VMPAYMENT_PAYPAL_AIO_PAYMENT_AMOUNT_INCORRECT'));
		//	return FALSE;
		//}
	}
	...
	/**
	 * No additional surcharge or fee is allowed for PayPal transactions
	 * @param VirtueMartCart $cart
	 * @param $cart_prices
	 * @param $activeMethod
	 * @return mixed
	 */
	function setCartPrices(VirtueMartCart $cart, &$cart_prices, $activeMethod) {
		if (empty($activeMethod->cost_per_transaction)) $activeMethod->cost_per_transaction = 0.0;
		if (empty($activeMethod->cost_percent_total) > 0.0) $activeMethod->cost_percent_total = 0.0;
		if (!empty($activeMethod->cost_percent_total) and !substr($activeMethod->cost_percent_total, 0, 1) == "-") {
			$activeMethod->cost_percent_total = "-" . $activeMethod->cost_percent_total;
		}

		if ($activeMethod->cost_per_transaction > 0.0) {
			$activeMethod->cost_per_transaction = -$activeMethod->cost_per_transaction;
		}
		return parent::setCartPrices($cart, $cart_prices, $activeMethod);
	}
