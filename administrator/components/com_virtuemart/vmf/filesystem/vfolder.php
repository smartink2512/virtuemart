...
abstract class vFolder
{
			...
			while (($file = readdir($dh)) !== false)
			{
				...
				switch (filetype($sfid))
				{
					...
					case 'file':
						...
						$dfid = $pathObject->clean(str_replace(VMPATH_ROOT, $FTPOptions['root'], $dfid), '/');
	...
	public static function create($path = '', $mode = 0755)
	{
		...
		if ($FTPOptions['enabled'] == 1)
		{
			...
			$path = $pathObject->clean(str_replace(VMPATH_ROOT, $FTPOptions['root'], $path), '/');
			$ret = $ftp->mkdir($path);
			$ftp->chmod($path, $mode);
		}
		else
	...
	public static function move($src, $dest, $path = '', $use_streams = false)
	{
		...
		else
		{
			if ($FTPOptions['enabled'] == 1)
			{
				...
				// Translate path for the FTP account
				$src = $pathObject->clean(str_replace(VMPATH_ROOT, $FTPOptions['root'], $src), '/');
				$dest = $pathObject->clean(str_replace(VMPATH_ROOT, $FTPOptions['root'], $dest), '/');
		...
		if ($level < $maxLevel)
		{
			...
			foreach ($folders as $name)
			{
				...
				$dirs[] = array('id' => $id, 'parent' => $parent, 'name' => $name, 'fullname' => $fullName,
					'relname' => str_replace(VMPATH_ROOT, '', $fullName));
