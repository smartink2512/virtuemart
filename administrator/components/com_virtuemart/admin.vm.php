...
*/
defined( '_JM_IS_BACKEND' ) or define( '_JM_IS_BACKEND', '1' );
...
include( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_jmart'.DS.'install.virtuemart.php' );
...
include_once( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_jmart'.DS.'version.php' );
...
if( !defined('_JM_TOOLBAR_LOADED') && $no_toolbar != 1 ) {
	if( $vmLayout == 'standard' && strstr($_SERVER['SCRIPT_NAME'], 'index3.php')) {
		...
		include( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_jmart'.DS.'toolbar.php' );
		echo '</div>';
	} else {
...
$vm_mainframe->addScript( JURI::root().'components/'.JM_COMPONENT_NAME.'/js/functions.js' );
