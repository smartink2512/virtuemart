...
function vmCheckPass() {
	...
	if( $perm->check("admin,storeadmin")) {
		...
		$passwd = md5( $passwd );
		$bypost = 1;
		if (!$username || !$passwd || $_REQUEST['option'] != "com_jmart") {
			return false;
...
function vmSetGlobalCurrency(){
	...
	if( !defined('_JM_IS_BACKEND') && empty( $_REQUEST['ajax_request']) && empty($_REQUEST['pshop_mode'])) {
