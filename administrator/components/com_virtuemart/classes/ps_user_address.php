...
class ps_user_address {
	...
	function validate_add(&$d) {
		...
		foreach( $shippingFields as $field )  {
			...
			if ( empty( $d[$field->name])) {
				$valid = false;
				$vmLogger->err(JText::_('JM_ENTER_VALUE_FIELD').' "'.(JText::_($field->title) != '' ? JText::_($field->title) : $field->title ).'"');
			}
		}
		if(empty($d['user_info_id'])) {
			...
			if ($db->next_record()) {
				...
				$vmLogger->warning( JText::_('JM_USERADDRESS_ERR_LABEL_EXISTS') );
				$valid = false;
			}
		}
	...
	function validate_delete(&$d) {
		global $vmLogger;
		if (empty($d["user_info_id"])) {
			$vmLogger->err( JText::_('JM_USERADDRESS_DELETE_SELECT') );
			return false;
		}
		else {
			return true;
	...
	function add(&$d) {
		...
		if( $db->query() === false ) {
			$GLOBALS['vmLogger']->err(JText::_('JM_USERADDRESS_ADD_FAILED'));
			return false;
		}
		$GLOBALS['vmLogger']->info(JText::_('JM_USERADDRESS_ADDED'));
	...
	function update(&$d) {
		...
		if( $db->query() === false ) {
			$GLOBALS['vmLogger']->err(JText::_('JM_USERADDRESS_UPDATED_FAILED'));
			return false;
		}
		$GLOBALS['vmLogger']->info(JText::_('JM_USERADDRESS_UPDATED'));
