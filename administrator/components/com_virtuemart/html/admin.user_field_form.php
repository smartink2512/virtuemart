...
$formObj = &new formFactory( JText::_('JM_USERFIELD_FORM_LBL') );
//Then Start the form
$formObj->startForm();
...
$webaddrtypes[0] = JText::_('JM_USERFIELDS_URL_ONLY');
$webaddrtypes[2] = JText::_('JM_USERFIELDS_HYPERTEXT_URL');
...
if (!empty($fieldid)) {
	...
	$types['text'] = JText::_('JM_FIELDS_TEXTFIELD');
	$types['checkbox'] = JText::_('JM_FIELDS_CHECKBOX_SINGLE');
	$types['multicheckbox'] = JText::_('JM_FIELDS_CHECKBOX_MULTIPLE');
	$types['date'] = JText::_('JM_FIELDS_DATE');
	$types['age_verification'] = JText::_('JM_FIELDS_AGEVERIFICATION');
	$types['select'] = JText::_('JM_FIELDS_DROPDOWN_SINGLE');
	$types['multiselect'] = JText::_('JM_FIELDS_DROPDOWN_MULTIPLE');
	$types['emailaddress'] = JText::_('JM_FIELDS_EMAIL');	
	$types['euvatid'] = JText::_('JM_FIELDS_EUVATID');
	$types['editorta'] = JText::_('JM_FIELDS_EDITORAREA');
	$types['textarea'] = JText::_('JM_FIELDS_TEXTAREA');
	$types['radio'] = JText::_('JM_FIELDS_RADIOBUTTON');
	$types['webaddress'] = JText::_('JM_FIELDS_WEBADDRESS');
	
	if( file_exists($mosConfig_absolute_path.'/administrator/components/com_securityimages/client.php')) {	
		$types['captcha'] = JText::_('JM_FIELDS_CAPTCHA');
	}
	if( file_exists($mosConfig_absolute_path.'/components/com_yanc/yanc.php')) {
		$types['yanc_subscription'] = JText::_('JM_FIELDS_NEWSLETTER').' (YaNC)';
	}
	if( file_exists($mosConfig_absolute_path.'/components/com_anjel/anjel.php')) {
		$types['anjel_subscription'] = JText::_('JM_FIELDS_NEWSLETTER').' (ANJEL)' ;
	}
	if( file_exists($mosConfig_absolute_path.'/components/com_letterman/letterman.php')) {
		$types['letterman_subscription'] = JText::_('JM_FIELDS_NEWSLETTER').' (Letterman)';
	}
	if( file_exists($mosConfig_absolute_path.'/components/com_ccnewsletter/ccnewsletter.php')) {
		$types['ccnewsletter_subscription'] = JText::_('JM_FIELDS_NEWSLETTER').' (ccNewsletter)';
	}
	$types['delimiter'] = JText::_('JM_FIELDS_DELIMITER');
...
if( in_array( $db->f('name'), ps_userfield::getSkipFields() )) {
	$lists['required'] = '<input type="hidden" name="required" class="inputbox" value="'. $db->sf('required').'" />'.($db->sf('required')?JText::_('JM_ADMIN_CFG_YES'):JText::_('JM_ADMIN_CFG_NO'));
	$lists['published'] = '<input type="hidden" name="published" class="inputbox" value="'. $db->sf('required').'" />'.($db->sf('required')?JText::_('JM_ADMIN_CFG_YES'):JText::_('JM_ADMIN_CFG_NO'));
	$lists['registration'] = '<input type="hidden" name="registration" class="inputbox" value="'. $db->sf('required').'" />'.($db->sf('required')?JText::_('JM_ADMIN_CFG_YES'):JText::_('JM_ADMIN_CFG_NO'));
} else {
...
?>
	<table cellpadding="4" cellspacing="0" border="0" width="100%">
		<tr>
			<td class="sectionname"><img src="<?php echo $mosConfig_live_site.'/administrator/images/addusers.png' ?>" align="middle"><?php echo JText::_('JM_MANAGE_USER_FIELDS') ?></td>
		</tr>
	</table>
	...
	<table class="adminform">
		<tr class="row0">
			<td width="20%"><?php echo JText::_('JM_FIELDMANAGER_TYPE') ?>:</td>
			<td width="20%"><?php echo $lists['type']; ?></td>
			<td>&nbsp;</td>
		</tr>
		<tr class="row1">
			<td width="20%"><?php echo JText::_('JM_FIELDMANAGER_NAME') ?>:</td>
		...
		<tr class="row0">
			<td width="20%"><?php echo JText::_('JM_FIELDMANAGER_REQUIRED') ?>?:</td>
			<td width="20%"><?php echo $lists['required']; ?></td>
			<td>&nbsp;</td>
		</tr>
		<tr class="row1">
			<td width="20%"><?php echo JText::_('JM_FIELDMANAGER_SHOW_ON_REGISTRATION') ?>?:</td>
			<td width="20%"><?php echo $lists['registration']; ?></td>
			<td>&nbsp;</td>
		</tr>
		<tr class="row0">
			<td width="20%"><?php echo JText::_('JM_FIELDMANAGER_SHOW_ON_ACCOUNT') ?>?:</td>
			<td width="20%"><?php echo $lists['account']; ?></td>
			<td>&nbsp;</td>
		</tr>
		<tr class="row1">
			<td width="20%"><?php echo JText::_('JM_FIELDMANAGER_SHOW_ON_SHIPPING') ?>?:</td>
			<td width="20%"><?php echo $lists['shipping']; ?></td>
			<td>&nbsp;</td>
		</tr>
		<tr class="row0">
			<td width="20%"><?php echo JText::_('JM_USERFIELDS_READONLY') ?>?:</td>
			<td width="20%"><?php echo $lists['readonly']; ?></td>
			<td>&nbsp;</td>
		</tr>
		<tr class="row1">
			<td width="20%"><?php echo JText::_('JM_FIELDMANAGER_PUBLISHED') ?>:</td>
			<td width="20%"><?php echo $lists['published']; ?></td>
			<td>&nbsp;</td>
		</tr>
		<tr class="row0">
			<td width="20%"><?php echo JText::_('JM_USERFIELDS_SIZE') ?>:</td>
		...
		<div id="divText">
			<table class="adminform">
			<tr class="row0">
				<td width="20%"><?php echo JText::_('JM_USERFIELDS_MAXLENGTH') ?>:</td>
		...
		<div id="divColsRows">
			<table class="adminform">
			<tr class="row0">
				<td width="20%"><?php echo JText::_('JM_USERFIELDS_COLUMNS') ?>:</td>
				<td width="20%"><input type="text" name="cols" mosLabel="Cols" class="inputbox" value="<?php echo $db->f('cols'); ?>" /></td>
				<td>&nbsp;</td>
			</tr>
			<tr class="row1">
				<td width="20%"><?php echo JText::_('JM_USERFIELDS_ROWS') ?>:</td>
		...
		<div id="divShopperGroups" >
			<table class="adminform">
	          <tr class="row1"> 
	        	<td class="labelcell"><?php echo JText::_('JM_USERFIELDS_EUVATID_MOVESHOPPER') ?>:</td>
	            <td ><?php
		...
		<div id="divAgeVerification" >
			<table class="adminform">
	          <tr class="row1"> 
	        	<td class="labelcell"><?php echo JText::_('JM_FIELDS_AGEVERIFICATION_MINIMUM') ?>:</td>
	            <td ><?php
		...
		<div id="divValues" style="text-align:left;height: 200px;overflow: auto;">
			<?php echo JText::_('JM_USERFIELDS_ADDVALUES_TIP') ?><br />
			<input type="button" class="button" onclick="insertRow();" value="<?php echo JText::_('JM_USERFIELDS_ADDVALUE') ?>" />
			<table align=left id="divFieldValues" cellpadding="4" cellspacing="1" border="0" width="100%" class="adminform" >
			<thead>
				<th class="title" width="20%"><?php echo JText::_('JM_USERFIELDS_TITLE') ?></th>
				<th class="title" width="80%"><?php echo JText::_('JM_USERFIELDS_VALUE') ?></th>
			</thead>
