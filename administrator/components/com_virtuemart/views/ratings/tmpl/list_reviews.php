...
<div style="text-align: left;">
	...
	if (!empty($this->reviewslist) and count($this->reviewslist) > 0) {
		...
		foreach ($this->reviewslist as $key => $review) {
			...
			$checked = vHtml::_('grid.id', $i , $review->virtuemart_rating_review_id ,null, 'virtuemart_rating_review_id');
			...
			<tr class="row<?php echo ($i)%2 ; ?>">
				...
				<td><?php echo vHtml::_('link', $link, $review->customer.' ('.vmJsApi::date($review->created_on,'LC2',true).')', array("title" => vmText::_('COM_VIRTUEMART_RATING_EDIT_TITLE'))); ?></td>
				...
				<td align="center">
					...
					<?php // echo vHtml::_('image', JURI::root().'/components/com_virtuemart/assets/images/stars/'.round($review->vote).'.gif',$review->vote,array("title" => (vmText::_('COM_VIRTUEMART_RATING_TITLE').' : '. $review->vote . ' :: ' . $this->max_rating)));
