...
$function	= vmRequest::getCmd('function', 'jSelectProduct');
        ...
        <div style="text-align: left;">
            ...
            <table class="adminlist" cellspacing="0" cellpadding="0">
                ...
                if ($total = count($this->productlist) ) {
                    $i = 0;
                    $k = 0;
                    $keyword = vmRequest::getString('keyword');
                    foreach ($this->productlist as $key => $product) {
                        ...
                        <tr class="row<?php echo $k ; ?>">
                            ...
                            <td align ="left>">
                                ...
                                <a class="pointer" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $product->virtuemart_product_id; ?>', '<?php echo $this->escape(addslashes($product->product_name)); ?>', '<?php echo 'index.php?option=com_virtuemart&view=product&virtuemart_product_id='.$product->virtuemart_product_id; ?>');">
                            ...
                            <?php if (!$product_parent_id ) { ?>
                                <td><?php
                                    if ($product->product_parent_id  ) {
                                        VirtuemartViewProduct::displayLinkToParent($product->product_parent_id,'&tmpl=component');
                                    }
                                    ?></td>
                                <!-- Vendor name -->
                            <?php } ?>
                            <td><?php
                                VirtuemartViewProduct::displayLinkToChildList($product->virtuemart_product_id , $product->product_name,'&tmpl=component');
                                ?>
                            </td>
                            <!-- Media -->
                            <?php
                            ...
                            <!-- Manufacturer name -->
                            <td><?php if ($product->virtuemart_manufacturer_id) {
                            ...
                            <td align="center" ><?php echo $product->reviews; ?></td>
                            <td align="center" >
                                <?php
                                echo $is_featured;
                                ?>
