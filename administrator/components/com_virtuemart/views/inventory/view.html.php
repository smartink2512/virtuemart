...
class VirtuemartViewInventory extends VmView {
	...
	function display($tpl = null) {
		...
		foreach ($inventorylist as $virtuemart_product_id => $product) {
			...
			if(!empty($product->prices['product_price'])){
				...
				$product->product_instock_value = '';
				$product->product_price_display = '';
				vmdebug('inventory view, the product '.$product->product_name.' id = '.$product->virtuemart_product_id.' has no price');
			}
		...
		$this->lists['stockfilter'] = JHTML::_('select.genericlist', $options, 'search_type', 'onChange="document.adminForm.submit(); return false;"', 'value', 'text', VmRequest::getVar('search_type'));
		$this->lists['filter_product'] = VmRequest::getVar('filter_product');
