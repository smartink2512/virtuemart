...
if( empty($vars['do_import'])) {
	echo '<h2>'.$VM_LANG->_('VM_CSV_UPLOAD_SIMULATION_RESULTS_LBL').'</h2>';
}
echo '<br />
...
if( empty( $vars['csv_import_finished'])) {
	...
	echo "<label for=\"csv_start_at\">".$VM_LANG->_('VM_CSV_UPLOAD_START_AT').": </label>
			<input class=\"inputbox\" type=\"text\" id=\"csv_start_at\" name=\"csv_start_at\" value=\"0\" size=\"6\" />
			<br /><br />";
	echo "<label for=\"csv_lines_to_import\">".$VM_LANG->_('VM_CSV_UPLOAD_LINES_TO_PROCESS').": </label>
	...
	echo "<input class=\"button\" type=\"submit\" name=\"submit\" value=\"".$VM_LANG->_('VM_CSV_UPLOAD_IMPORTNOW')."\" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n";
	echo "<input class=\"button\" type=\"button\" name=\"cancel\" value=\"".$VM_LANG->_('CMN_CANCEL')."\" onclick=\"document.location='".$sess->url($_SERVER['PHP_SELF']."?page=product.csv_upload")."';\" />\n";
...
if( !empty( $vars['error_log'] ) && is_array($vars['error_log'])) {
	...
	echo '<h3><img src="'.$mosConfig_live_site.'/administrator/images/tick.png" hspace="5" alt="ok" />'.$VM_LANG->_('VM_CSV_UPLOAD_NO_ERRORS').'</h3>';
}

echo '<td valign="top" width="50%" rowspan="2">
		<fieldset><legend>'.$VM_LANG->_('VM_CSV_UPLOAD_DETAILS_ANALYSIS').'</legend>';
if( empty( $vars['do_import'])) {
	echo '<strong>'.$VM_LANG->_('VM_CSV_UPLOAD_TOTAL_LINES').': '.$vars['csv_log']['total_lines'].'</strong><br/><br />';
}
echo '<strong>'.$VM_LANG->_('VM_CSV_UPLOAD_FIRST_LINE').':</strong>
<div style="width:400px;overflow:auto;" class="quote">
<pre>'.htmlspecialchars($vars['csv_log']['first_line_raw']).'</pre>
</div>';

echo '<strong>'.$VM_LANG->_('VM_CSV_UPLOAD_FIELD_EXPLANATION').' :</strong>';
$i = 0;
echo '<ol>';
...
echo '<fieldset><legend>'.$VM_LANG->_('VM_PRODUCT_IMPORT_LOG').'</legend>';
